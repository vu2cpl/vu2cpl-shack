...
| # | Item | Status |
|---|------|--------|
| 45 | **Rotator overlap — :8090 compass page** (opened 2026-09-08) — ~~HA entity~~ **landed the same evening on operator request**: `binary_sensor.rotator_overlap` via `ha_discovery_publish.py` + a conditional amber ⟳ icon on the Radio dashboard's compass card (verified live, entity reading the real flag). Remaining: rotator-remote's `:8090` standalone compass doesn't render the flag it already receives (small `web/app.js` addition) | HA done; :8090 pending |
