...
| 15 | AS3935 "Last seen" survives restart via retained `lightning/as3935/last_event` topic. | **Done 2026-05-15** — both ends verified end-to-end. Firmware side: `vu2cpl-as3935-bridge` commit `f895d71` adds `publishLastEvent()` with `retain=true`, flashed to ESP32 at ~07:39 IST. First disturber at 07:44:28 IST published payload `{event:"disturber", distance:0, energy:0, timestamp:"2026-05-15T07:44:28", ts_epoch_ms:1778811268000}` to broker; retained. Node-RED side: shack commit `f75e147` consumes the retained payload via `as3935_last_event_mqtt_in` → `as3935_format_state` → Master Dashboard handler, which seeds `as3935LastTs` from `ts_epoch_ms` and paints relative age (`Ns ago` / `Nm ago`) with absolute IST tooltip on hover. Confirmed working on Node-RED restart. |
| 16 | Dashboard rehydration audit. | **Done 2026-05-15** — audit revealed most tabs were already fine. 11 of 12 widgets across the 10 remaining tabs have both `storeOutMessages: true` + `resendOnRefresh: true` AND are fed by a single "State Aggregator" function that emits the **full state object** on every update (not partial diffs). Browser refresh replays the last full-state message → widget paints completely. The AS3935 Tuning Control Panel from 2026-05-14 was special because its widget dispatched on `msg.topic` with three separate input topics (`/status`, `/hb`, `/cmd_ack`), so `resendOnRefresh` could only replay one of them. **Three aggregators with slow-poll sources** had `flow.set` / `flow.get` on **memory** context (wiped on restart): Solar State Aggregator (`d8ed9693432963df`), RBN State Aggregator (`713be21e706f5f9c`), RPi State Aggregator (`ee59f04cdd4ea9c6`). All three switched to `'file'` scope on both the get and the set (`enable_file_context.sh` made `'file'` scope available per TODO #6). State now survives Node-RED restart. Power Strips has its own `Poll Tasmota state every 10s` inject that keeps state fresh continuously — no fix needed. |
