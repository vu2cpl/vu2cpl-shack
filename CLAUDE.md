...
| RBN Skimmer Monitor | `f9a0e3ad0e019052` | 23 | `1bcbc2eb8f2124aa` |
...
| UberSDR | `ubersdr_tab` | 10 | `ubersdr_grp` (on Shack Monitoring tools) |
...
| `1edeb9703cae2fcb` | RBN Skimmer Panel | ui_template — live spot stream + per-skimmer status. Since 2026-09-09 also a collapsible `<details>` "FT8/FT4 Decodes — UberSDR" table (closed by default; only the tbody repaints, so the open state survives) fed by `payload.ft8`. |
| `rbn_ft8_mqtt_in` | ubersdr/metrics/digital_modes/# | `mqtt in` (broker `f4785be9863eab08`) — UberSDR's per-decode FT8/FT4/WSPR stream: `{band, callsign, country, frequency, mode, snr, message, timestamp}` per decode. Added 2026-09-09. |
| `rbn_ft8_parse_fn` | Parse UberSDR Digital Decode | Keeps `flow.ft8_decodes` (memory ring, cap 300) and pokes `RBN State Aggregator` so decodes paint promptly. The aggregator + `vue_rbn_builder_01` attach `ft8:{decodes: last 30, h1, h1capped}` to their **outgoing payloads only** — deliberately not into file-scoped `rbn_dash`, keeping the `shack/rbn/state` HA bridge small. |
