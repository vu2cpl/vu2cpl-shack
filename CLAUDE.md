...
| 34 | **SPE Tune — proper button + indicator reflecting the amp's front-panel TUNE LED.** Make SPE Tune a real button on both dashboards (Vue SPECard already has `confirmTune()` + SWEEP panel @ `uibuilder/shack/src/index.js:1525`; D1 `ws_panel_node` has tune/ATU refs — bring to parity) and add an indicator that mirrors the amp's TUNE LED (tuning/tuned). New work = surface the TUNE-LED bit from the 76-byte SPE frame in `vu2cpl/spe-remote` → `ws_format_state` → both UIs. Anchors: `atuswr`/`speTuneBand`/`speTuneStop`. See HANDOVER #34. **Implemented 2026-06-19** (Node-RED + Vue only — spe-remote already decodes + broadcasts `tune_active`): `ws_format_state` emits `tune`, D1 Tune button has `#spew-tune-led`, Vue Tune button glows + `⚡ TUNE` header chip. **Still open — awaiting on-amp TUNE-cycle verification.** | Impl · verify |
