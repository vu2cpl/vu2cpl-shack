...
| `stabiliser_watch.py` | `/home/vu2cpl/stabiliser_watch.py` (user crontab `* * * * *` `--cron`) | Servo-stabiliser watchdog → Telegram. Tails `~/grid_voltage.csv`; alerts on transition into **outage** (grid absent 2+ samples — the nightly-cut pattern under test), **dropout** (vmax > `RAW_MAINS_V` 245 V for 3+ samples = stabiliser out of circuit), or **stale** (no new row 6 min), plus a ✅ recovery. State in `~/.stabiliser_watch.state` so each transition alerts once. Phase *spread* is deliberately not the discriminator — it overlapped completely between regulated and bypassed operation; absolute voltage separates them (regulated 230–236 V vs raw mains 250–264 V). Telegram creds resolve env → shack env files (`/etc/default/vu2cpl-shack`, then `~/.config/vu2cpl-shack.env` which wins — monitor.sh's precedence) → Node-RED's `/proc/<pid>/environ` as fallback, so **alerting survives Node-RED being down** (env-file step added 2026-08-29; `flows_guard.py` got the same). `--status` / `--test` for checks. Added 2026-08-29 |
