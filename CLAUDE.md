...
- CSV logging enabled
- **Band sweep is self-managing since 2026-09-02** (spe-remote `a1c1252`, TODO #44): the orchestrator auto-switches the amp to STBY at sweep/single start (restoring OPERATE at the end iff it was on — every exit path incl. ABORT) and verifies the requested band against the Flex slice freq before keying anything (mismatch FAILs with `band mismatch:` in the tune_event; `tune_band:auto` sweeps the radio's current band). New phases `BAND_CHECKED`/`STBY_SET`/`OPER_RESTORED` pass through `ws_format_state`'s tune_event path untouched — both dashboards render phases generically, so only the sweep hint/confirm copy changed (D1 `SPE Panel (WS)` + Vue `v33`): don't reintroduce "amp must be in STBY" wording, the server handles it.
