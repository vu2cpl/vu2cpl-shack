...
| # | Item | Status |
|---|------|--------|
| 41 | **HA Radio dashboard phase 2** — the Flex/SPE/LP-700/Rotator/DXCC/RBN cards need a Node-RED→MQTT state bridge (flow change ⇒ rule #1 approval); the bridge should also publish bypass *state* (HA's BYP buttons currently fire blind). Phase 1 (7 data-ready cards, 8 sections, 108 entities) live at `/193-radio` since 2026-08-25; lightning ANT/BYPASS controls wired same evening via `rest_command.shack_*` (basic-auth `!secret nodered_http_password` in HassPi's `secrets.yaml`; note `reload_all` won't load a brand-new YAML domain — core restart needed). See HANDOVER #41 | Pending (part done) |
