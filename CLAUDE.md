...
| # | Item | Status |
|---|------|--------|
| 44 | **SPE sweep tuning — auto-STBY, band check, OP restore** (opened + scoped 2026-09-02). The sweep already exists in **`spe-remote`** (`spe/tune_orchestrator.py`, v3.0.0, 2026-06): Node-RED just sends `tune_band:<band>` / `tune_single` / `tune_stop` over the WS and renders `tune_event` phases; VFO freq+mode save/restore is already done. Gaps to close, all in `TuneOrchestrator`: (1) auto-switch the amp to STBY at start (today it FAILs unless already in STBY), remembering prior mode; (2) verify the sweep band matches the radio's current band (today the operator's menu pick is trusted blindly); (3) restore OPERATE at the end iff it was on at the start. Work lands in the spe-remote repo, not this one. See HANDOVER #44 | Pending |
