...
| ID | Name | Role |
|----|------|------|
| `df7d1786eab4d5a2` | Telnet VU2CPL :7550 | `tcp in` client, port 7550. **Repointed 2026-07-31** from `vu2cpl.ddns.net` to `192.168.1.109` — direct LAN IP, skipping the DDNS+router-port-forward round trip. **What answers on `:7550` changed too** (same day's discovery): it's now **meridian's DX-cluster aggregator server** (`meridian-core/src/dxcluster/server.rs` on the ubersdr box, fed by the CW Skimmer test on the Windows box), not CwSkimmer's raw no-login port — and it **requires a login** (`Welcome to Aggregator … Please enter your callsign:\r\n`). `Login Handler VU2CPL`'s prompt detection now takes the last **non-empty** line of the chunk (2026-07-31 fix): meridian terminates its prompt with `\r\n`, which made the literal last split element an empty string, so the login was never sent and both VU2CPL connections sat at the prompt forever (root cause of "red tile, no spots"). CwSkimmer's unterminated prompt style still detects too. Host history: `vu2cpl.ddns.net:7300` (auth port) 2026-05-17→2026-07-13, DDNS `:7550` until 2026-07-31, LAN `.109:7550` since. |
