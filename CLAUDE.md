...
| # | Item | Status |
|---|------|--------|
| 44 | **SPE sweep tuning — automated ATU tune sweep across the band in use** (opened 2026-09-02). Sequence per operator: remember OP/STBY + current radio frequency → put amp in **STANDBY** first → verify the amp's band matches the band currently in use → run the sweep → return the radio to the original frequency → re-enable **OPERATE** only if it was on at the start. Needs both `spe-remote` (STBY/OP, band, TUNE; `tune_active` already broadcast) and FlexRadio TCP (frequency stepping + low-power tune carrier). Open: where the orchestration lives (spe-remote vs Node-RED vs standalone script), segment step size (ATU stores per-segment memories), abort/safety (SWR fault mid-sweep, operator interrupt, restore-on-failure). See HANDOVER #44 | Pending |
