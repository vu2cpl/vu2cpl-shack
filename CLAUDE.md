...
| ID | Name | Role |
|----|------|------|
| `df7d1786eab4d5a2` | Telnet VU2CPL :7550 | `tcp in` client to `vu2cpl.ddns.net:7550` — CwSkimmer's "raw" no-login port. **Reverted here 2026-07-13** (was `:7300`, the auth-required port, since 2026-05-17) because `:7300` started returning `ECONNREFUSED` — the auth port isn't currently reachable from outside. `newline: ""` (raw chunks) is a harmless carryover from the `:7300` era, when it was mandatory because CwSkimmer's `Please enter your callsign:` prompt has no trailing `\n`; on `:7550` there's no prompt at all, so `Login Handler VU2CPL` below just no-ops and passes spots straight through (same as `Login Handler VU2OY`). |
