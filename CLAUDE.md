...
| ID | Name | Role |
|----|------|------|
| `df7d1786eab4d5a2` | Telnet VU2CPL :7550 | `tcp in` client, port 7550. **Repointed 2026-07-31** from `vu2cpl.ddns.net` to `192.168.1.109` — direct LAN IP for the ubersdr box (CW Skimmer test feed), skipping the DDNS+router-port-forward round trip. History: was `vu2cpl.ddns.net:7300` (auth-required port) 2026-05-17→2026-07-13, reverted to the DDNS `:7550` "raw" no-login port after `:7300` started returning `ECONNREFUSED` from outside; now off DDNS entirely since the feed is reachable directly on the LAN. `newline: ""` (raw chunks) is a harmless carryover from the `:7300` era, when it was mandatory because CwSkimmer's `Please enter your callsign:` prompt has no trailing `\n`; there's no login prompt on this feed regardless of host, so `Login Handler VU2CPL` below just no-ops and passes spots straight through (same as `Login Handler VU2OY`) — unaffected by the host change, since that behavior depends on the skimmer software, not the network path. |
...
- DX Clusters: N2WQ (`cluster.n2wq.com:8300`), VU2OY (`vu2oy.ddns.net:7550`), VU2CPL (`vu2cpl.ddns.net:7550`), VE7CC (`ve7cc.net:23`) — VU2CPL's port history: moved 7550 → 7300 on 2026-05-17 (commit `4b1fabb`, to reach CwSkimmer's auth-required cluster port), then **reverted 7300 → 7550 on 2026-07-13** (commit `98daacb`) after `:7300` started returning `ECONNREFUSED` from outside — currently on the "raw" no-login port again. Both DXCC Tracker (`login-parse-dedup-v2`) and RBN Skimmer Monitor (`Login Handler VU2CPL`) still carry the `VU2CPL-1\r\n` login-prompt-detection code from the `:7300` era; on `:7550` there's simply no prompt to match, so it no-ops and spots pass straight through, same as VU2OY's connection always has. tcp-out reply nodes use `beserver:reply` + `_session` to write back on the same socket regardless of port. See SHACK_CHANGELOG.md 2026-05-17 "VU2CPL skimmer — login handlers" for the original auth-port story and gotchas (notably `newline:""` on the tcp-in, a harmless carryover now that there's no prompt), and 2026-07-13 for the revert + the stale-editor-tab incident that prompted re-checking this whole area.
