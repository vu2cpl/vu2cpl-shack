...
| `shack/rotator/state` | `rot_state_{tick,fn,mqtt}_01` | Rotator | `flow.rotator_heading` + `flow.target_hdg` + `global.rotator_power` (since 2026-09-01; `null` until first known) + `flow.rotator_overlap` (since 2026-09-08; no HA entity yet — TODO #45) | 5 s |
...
| # | Item | Status |
|---|------|--------|
| 45 | **Rotator overlap — HA entity + :8090 compass page** (opened 2026-09-08) — the gateway-derived `overlap` flag already rides `shack/rotator/state`, so the HA half is one `binary_sensor` discovery config in `ha_discovery_publish.py` + a Radio-dashboard tweak; rotator-remote's `:8090` standalone compass also doesn't render the flag yet (small `web/app.js` addition). Deferred on operator scope call ("both dashboards now, HA later") | Pending |
