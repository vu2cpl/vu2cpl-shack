...
| # | Item | Status |
|---|------|--------|
| 42 | **Stabiliser watchdog loose ends** — (a) `stabiliser_watch.py` reads its Telegram creds from Node-RED's `/proc/<pid>/environ`, so **alerts stop if Node-RED is down** (same gap as `flows_guard.py`); fix = token into `~/.config/vu2cpl-shack.env`, operator's call. (b) `RAW_MAINS_V = 245` was calibrated on ~4 min of regulated output — confirm against a full night before trusting the dropout branch. See HANDOVER #42 | Pending |
...
| `stabiliser_watch.py` | `/home/vu2cpl/stabiliser_watch.py` (user crontab `* * * * *` `--cron`) | Servo-stabiliser watchdog → Telegram. Tails `~/grid_voltage.csv`; alerts on transition into **outage** (grid absent 2+ samples — the nightly-cut pattern under test), **dropout** (vmax > `RAW_MAINS_V` 245 V for 3+ samples = stabiliser out of circuit), or **stale** (no new row 6 min), plus a ✅ recovery. State in `~/.stabiliser_watch.state` so each transition alerts once. Phase *spread* is deliberately not the discriminator — it overlapped completely between regulated and bypassed operation; absolute voltage separates them (regulated 230–236 V vs raw mains 250–264 V). Telegram creds via the `flows_guard.py` trick (Node-RED's `/proc/<pid>/environ`), so **alerts are skipped if Node-RED is down**. `--status` / `--test` for checks. Added 2026-08-29 |
