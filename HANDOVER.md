...
| 05-10 | Chrony / GPS Time Server card | New transitional dashboard for `gpsntp.local` (the stratum-1 GPS-disciplined NTP server installed 2026-05-09). What landed in `743a0d8`: a 7-widget version on a dedicated `GPS NTP` dashboard tab (id `4cac0c07e2686c33`) — mqtt-in `shack/gpsntp/chrony` → `split + format` function → 6 ui_text/ui_gauge widgets. **Target state** (per [`pi-gps-ntp-server`](https://github.com/vu2cpl/pi-gps-ntp-server) `dashboard/`): single `ui_template` on the existing fleet panel, no intermediate function — all formatting + threshold logic in the template's AngularJS `<script>`. Migration path documented in CLAUDE.md and SHACK_CHANGELOG: in-place edit of the ui_template's format box with `dashboard/chrony-card-template.html`. Will then delete the dedicated `GPS NTP` tab. Topic is retained, JSON, refreshed once/min by `/usr/local/bin/gpsntp-mqtt-publish.sh` (Pi-side cron, NOT in this repo). |
