...
| 20 | DXCC worked-table dual-write bug | Surfaced while investigating #19 — `Fetch All Modes + Parse` writes `workedTable`/`entityWorked`/`dxccModeWorked`/`workedStats` to `'file'` scope only; downstream readers (`DXCC Prefix Lookup`, `Return Stats`) read with no scope (memory). Before today the 'file' store was a silent no-op so writes were lost, masked by other ways the in-memory data got populated. Now that the file store is live, post-fetch the file gets fresh data but the running session's memory does not — the running classifier still uses the previous start's memory snapshot until next Node-RED restart (which then Bootstraps file→memory). Investigation + fix needed: either dual-write (memory + file) in the Fetch function or have the consumers read from file. Lower priority than it sounds because the daily fetch is typically followed by no restart, so the lag is at most ~24 h; but worth a cleanup |
