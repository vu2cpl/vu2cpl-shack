...
| 21 | AS3935 outdoor sensor — ESP32 firmware project | Sister project to support HANDOVER #1 (AS3935 outdoors). Planned new repo `vu2cpl-as3935-bridge` (`~/projects/vu2cpl-as3935-bridge/`, eventual `github.com/vu2cpl/vu2cpl-as3935-bridge`). ESP-WROOM-32 + AS3935 (I²C) + 18650 + TP4056 + solar, in a shade-mounted IP65 enclosure, WiFi → MQTT to the existing broker. **Identical MQTT contract** to the indoor `as3935_mqtt.py` daemon so the Node-RED Lightning Antenna Protector flow needs zero changes. Design decisions captured (ESP32 over ESP8266; PlatformIO + Arduino; always-on WiFi in v0.1.0 with deep-sleep deferred; `AFE_GB=0x1C` outdoor; TUN_CAP re-sweep post-install; CALIB_RCO at startup; LWT armed). 7-file scaffold (README, HANDOVER, BUILD, platformio.ini, src/main.cpp, LICENSE, .gitignore) drafted 2026-05-11; operator to drop into the new repo when ready. HANDOVER #1 stays open until the hardware actually moves outside |
