...
| 05-10 | DXCC: startup fetch + Bootstrap resilience | Two interlocking bugs surfaced after the secrets redeploy left the tracker paused. (1) `Load Club Log on startup` and `Retry Club Log (90s)` injects had `once: false` — `onceDelay` set but ignored, so neither fired on deploy. Only the 02:00 cron + manual button could trigger Club Log fetch. Fix: ticked once-on-startup; now fires at +12 s with +90 s safety retry. (2) `dxccReady` flag was only set by successful Club Log fetch, so any auth/network blip left the tracker paused indefinitely even with cached data already loaded from `nr_dxcc_seed.json`. Fix: `Bootstrap Worked Table` now flips `dxccReady=true` in all three success branches (file-store, already-loaded, seed-file). Tracker is operational within 2 s of restart regardless of Club Log availability; live fetch overlays fresh data later (idempotent). Closes the symptom of HANDOVER follow-up #11. |
