...
| AS3935 publisher | ✅ ESP32 bridge ([`vu2cpl-as3935-bridge`](https://github.com/vu2cpl/vu2cpl-as3935-bridge) v0.1.1) live since 2026-05-11, bench bring-up complete, indoor `as3935.service` on noderedpi4 stopped + disabled (kept as fallback). MQTT contract unchanged so Node-RED Lightning Antenna Protector flow needed zero edits |
...
| 21 | AS3935 outdoor sensor — ESP32 firmware project | **v0.1.1 live on the bench 2026-05-11** ([`vu2cpl-as3935-bridge`](https://github.com/vu2cpl/vu2cpl-as3935-bridge)). ESP-WROOM-32 + AS3935 (I²C) running, `TRCO=OK SRCO=OK`, MQTT pipe to `192.168.1.169:1883` live, WiFiManager captive-portal credential entry replaced the original `secrets.h` approach, retained status auto-refreshes every 5 min, hb every 30 s. Bench-verified: piezo-lighter sparks produce `event:"disturber"` events the Node-RED flow consumes unchanged. Indoor `as3935.service` on noderedpi4 stopped+disabled — ESP32 is the sole publisher. Open for v0.2.0+: on-device TUN_CAP calibration mode (port `as3935_tune.py`), power chain (TP4056 + 18650 + solar), enclosure seal, field install with in-situ TUN_CAP re-tune. Closes the firmware half of HANDOVER #1; the hardware-outdoors half stays open until physically installed |
