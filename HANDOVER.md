...
| 12 | ~~Install `log2ram` on `gpsntp`~~ | **Done 2026-05-11** — azlux repo (`trixie main`, not `bookworm main` as BUILD.md says — gpsntp moved to Debian 13), `log2ram` 1.7.x installed + enabled, rebooted. `/var/log` now tmpfs 128 MB; chrony re-locked to stratum 1 with PPS within ~60 s of boot. **Action item:** `pi-gps-ntp-server/BUILD.md` line 497 still hardcodes `bookworm main` — separate repo, follow-up there |
